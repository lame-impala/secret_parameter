...
class AbstractPacker
  ...
  def check_string str
    raise PackerError.new("Expected string, got #{str.class.name}") unless str.is_a? String
    raise PackerError.new("String too short, expected #{min_bytes}, got #{str.bytesize}") if (str.bytesize < min_bytes)
    raise PackerError.new("String too long, expected #{max_bytes}, got #{str.bytesize}") if (!max_bytes.nil? && str.bytesize > max_bytes)
  end
...
class AbstractUintPacker < AbstractPacker
  def check_value uint
    raise PackerError.new("Expected integer, got #{uint.class.name}") unless uint.is_a? Integer
    raise PackerError.new("Expected non-negative integer, got #{uint}") if (uint < 0)
    exp = num_bytes * 8
    limit = 2 ** exp
    raise PackerError.new("Expected integer below #{limit}, got #{uint}") if (uint >= limit)
  end
  def pack value
    check_value value
